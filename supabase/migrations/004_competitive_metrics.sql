...
-- Migration 004: Add Competitive Metrics Tables
...
CREATE TABLE IF NOT EXISTS public.mission_records (
    ...
    fastest_time_holder UUID REFERENCES public.users(id),
    fastest_time_date TIMESTAMPTZ,
    
    -- Quality Records
    highest_quality_score INTEGER,
    highest_quality_holder UUID REFERENCES public.users(id),
    highest_quality_date TIMESTAMPTZ,
    
    -- Innovation Records
    highest_innovation_score INTEGER,
    highest_innovation_holder UUID REFERENCES public.users(id),
...
BEGIN
    ...
    -- Award XP to user profile if users table has total_xp column
    UPDATE public.users
    SET total_xp = COALESCE(total_xp, 0) + v_points,
        updated_at = CURRENT_TIMESTAMP
    WHERE id = p_user_id;
